...
class EstimationAgent(ideal_world.Agent):
    def __init__(
            self,
            time_interval,
            nu,
    ...
    def observation_update(
        ...
        for d in observation:
            ...
            particle_suggest_pos =\
                ideal_world.IdealCamera.observation_function(
                    self.pose,
                    pos_on_map)
...
class Mcl:
    def __init__(
        ...
        v = motion_noise_stds
        c = np.diag([v["nn"], v["no"], v["on"], v["oo"]])**2
    ...
    def draw(self, ax, elems):
        ...
        vxs = [math.cos(p.pose[2])*p.weight*particle_num
               for p in self.particles]
        vys = [math.sin(p.pose[2])*p.weight*particle_num
               for p in self.particles]
        elems.append(
            ax.quiver(
                xs,
