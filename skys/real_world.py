from . import ideal_world
from scipy.stats import expon, norm, uniform
import math
import numpy as np


class Robot(ideal_world.IdealRobot):
    def __init__(
            self,
            pose,
            agent=None,
...
class Camera(ideal_world.IdealCamera):
    def __init__(
        self,
        env_map,
        ...
        # 距離のバイアス、方向のバイアス
        self.distance_bias_rate_std = norm.rvs(
            scale=distance_bias_rate_stddev)
        self.direction_bias = norm.rvs(
            scale=direction_bias_stddev)
    ...
    def bias(self, relpos):
        return relpos+np.array(
            [relpos[0]*self.distance_bias_rate_std,
                self.direction_bias]
        )
